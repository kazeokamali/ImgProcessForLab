...
class BlacklineSettingsWidget(QWidget):
    def __init__(self, parent=None):
        ...
        self.confirm_button.setStyleSheet("background-color: #0078d4; color: white; font-weight: bold; padding: 10px;")
...
class BlacklineInterface(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.cancel_button = IconTextButton("清空选择", "./resource/icons/clear.png", self)
        ...
        self.batch_size_slider.setFixedSize(180, 30)
        self.batch_size_slider.valueChanged.connect(self.on_batch_size_changed)
        button_layout.addWidget(self.batch_size_slider)
        
        self.batch_size_label = QLabel("100")
        self.batch_size_label.setStyleSheet("color: #0078d4; font-weight: bold;")
        self.batch_size_label.setFixedWidth(20)
        button_layout.addWidget(self.batch_size_label)
        ...
        button_layout.addWidget(self.gpu_settings_button)
        
        button_layout.addWidget(QLabel("背底处理:"))
        button_layout.addWidget(self.process_method_combo)
        button_layout.addWidget(self.process_background_button)
        button_layout.addWidget(self.process_tomos_button)
        button_layout.addWidget(self.process_blackline_only_button)
        button_layout.addWidget(self.process_grad_button)
    ...
    def open_gpu_settings(self):
        ...
        def apply_settings():
            try:
                ...
                QMessageBox.information(dialog, "成功", f"GPU设置已更新！\n显存使用率: {memory_value}%\nCUDA流数量: {stream_value}\n批次大小: {batch_value}")
                dialog.accept()
            except ValueError:
    ...
    def process_background(self):
        ...
        if not os.path.exists(input_folder_background):
            QMessageBox.warning(
                self, "Warning", "输入文件夹下缺少 Background 子文件夹！"
            )
            return
    ...
    def process_white_black(self, input_folder, output_folder):
        ...
        if not white_files or not black_files:
            QMessageBox.warning(
                self, "Warning", "在输入文件夹中未找到合适的白场或黑场文件！"
            )
            return
        ...
        self.text.append(f"参数设置 - Min: {min_val}, Max: {max_val}, Grad: {grad}, Blackline_lp: {blackline_lp}")
        ...
        self.text.append(f'<span style="color: green;">成功！处理后的图像已保存到: {save_path}</span>')
        self.text.append(f"-----------------------------------------------------------")
    ...
    def process_average_white_black(self, input_folder, output_folder):
        ...
        self.text.append(f'<span style="color: green;">成功！平均白场已保存到: {save_path}</span>')
        ...
        self.text.append(f'<span style="color: green;">成功！平均黑场已保存到: {save_black_path}</span>')
    ...
    def process_tomos(self):
        ...
        if not os.path.exists(input_folder_tomo):
            QMessageBox.warning(
                self, "Warning", "输入文件夹下缺少 Tomos 子文件夹！"
            )
            return
        ...
        if not os.path.exists(input_folder_wsb):
            QMessageBox.warning(
                self, "Warning", "输出文件夹下缺少 Folder1（白减黑结果）！请先运行处理背底功能。"
            )
            return
        ...
        if os.path.exists(avg_black_path):
            ...
            black_files = self._find_keyword_tiff_files(input_folder_background, "black")
            if not black_files:
                QMessageBox.warning(
                    self, "Warning", "在 Background 中未找到黑场文件（名称需包含 black）！"
                )
                return
            if len(black_files) > 1:
                self.text.append(
        ...
        if not tomo_files:
            QMessageBox.warning(
                self, "Warning", "Tomos 子文件夹下没有可处理的 .tif/.tiff 文件！"
            )
            return
        ...
        if white_sub_black_img_path is None:
            white_sub_black_files = self._find_keyword_tiff_files(input_folder_wsb, "white_sub_black")
            if not white_sub_black_files:
                QMessageBox.warning(
                    self, "Warning", "在输出文件夹中未找到白减黑图像！"
                )
                return
            white_sub_black_img_path = os.path.join(input_folder_wsb, white_sub_black_files[0])

        self.text.append(f"使用白减黑文件: {os.path.basename(white_sub_black_img_path)}")
        ...
        processor = GPUBatchProcessor(memory_usage_percent=self.gpu_memory_usage, num_streams=self.cuda_stream_count, batch_size=self.batch_size)
        ...
        processor.process_divide_batch(tomo_result_files, white_sub_black_img_path, folder3_dir, progress_callback_step2)

        self.text.append(f'<span style="color: green;">处理完成！结果保存在: {folder3_dir}</span>')
        self.text.append(f"-----------------------------------------------------------")

    def process_blackline_only(self):
        self.text.append(f"----------------------------------------")
        self.text.append("Start blackline-only processing (no background correction)...")

        input_folder = self.input_path
        output_folder = self.output_path
        if not input_folder or not output_folder:
            QMessageBox.warning(
                self, "Warning", "Please select input and output folders first."
            )
            return
        ...
        if not image_todo_files:
            QMessageBox.warning(
                self, "Warning", "No .tif/.tiff files found in input folder."
            )
            return
        ...
        if settings["mode"] == "txt_file":
            ...
            if not columns:
                QMessageBox.warning(
                    self,
                    "Warning",
                    "No blackline column data found. Select a txt file or switch to auto mode.",
                )
                return
            self.text.append(f"Mode: txt_file, columns count: {len(columns)}")
        else:
            if_process_all = True
            columns = None
            self.text.append("Mode: auto_detect (process all columns).")

        self.text.append(f"Found {len(image_todo_files)} input images.")
        self.text.append(
            f"Params - Min: {min_val}, Max: {max_val}, Blackline_lp: {blackline_lp}"
        )
        ...
        success_count = 0
        fail_count = 0
        for image_todo in tqdm.tqdm(image_todo_files, desc="Blackline-only Processing"):
            try:
                image_todo_img = GPUImageProcess(image_todo)
                image_todo_img.min_val = min_val
                image_todo_img.max_val = max_val
                ...
                image_todo_img.delete_blacklines_inRange()

                basename = os.path.basename(image_todo)
                if basename.lower().endswith(".tiff"):
                    save_path = os.path.join(save_dir, basename.replace(".tiff", "_.tiff"))
                elif basename.lower().endswith(".tif"):
                    save_path = os.path.join(save_dir, basename.replace(".tif", "_.tif"))
                else:
                    save_path = os.path.join(save_dir, basename)

                image_todo_img.save_to_path(save_path)
                self.text.append(
                    f'<span style="color: green;">Success: {os.path.basename(image_todo)}</span>'
                )
                success_count += 1
            except IOError:
                self.text.append(
                    f'<span style="color: red;">Warning: cannot process file {image_todo}, skipped.</span>'
                )
                fail_count += 1
            except Exception as e:
                self.text.append(
                    f'<span style="color: red;">Failed: {image_todo}, reason: {e}</span>'
                )
                fail_count += 1

        self.text.append(
            f'<span style="color: green;">Blackline-only complete. Success: {success_count}, '
            f'Failed: {fail_count}, Output: {save_dir}</span>'
        )
        self.text.append(f"-----------------------------------------------------------")
    ...
    def process_img_inGrad(self):
        ...
        for image_todo in tqdm.tqdm(image_todo_files, desc="梯度处理"):
            try:
                ...
                if basename.endswith('.tiff'):
                    ...
                    save_path = os.path.join(save_dir, basename)
                image_todo_img.save_to_path(save_path)
                self.text.append(f'<span style="color: green;">成功: {os.path.basename(image_todo)}</span>')
        ...
        self.text.append(f'<span style="color: green;">梯度方法处理完成！结果保存在: {os.path.join(output_folder, "Folder_inGrad")}</span>')
        self.text.append(f"-----------------------------------------------------------")
